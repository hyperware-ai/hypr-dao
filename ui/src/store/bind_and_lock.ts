...
import type { BindAndLockState, LockStatusPayload } from '../types/bind_and_lock';
import { getNodeId } from '../types/global';
import { App } from '#caller-utils';

interface BindAndLockStore extends BindAndLockState {
  initialize: () => void;
...
export const useBindAndLockStore = create<BindAndLockStore>((set, get) => ({
  nodeId: null,
  isConnected: false,
  ownerAddress: null,
  lockDetails: null,
...
function applyStatus(status: LockStatusPayload, set: (updater: Partial<BindAndLockState>) => void) {
  set({
