...
export async function fetchQuorumProgress(proposalId: string): Promise<QuorumProgress> {
  const response = await fetch(`${API_PREFIX}/api/quorum-progress`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ QuorumProgress: proposalId }),
  });
  if (!response.ok) {
    throw new Error(`Failed to load quorum (status ${response.status})`);
  }
  const json = await response.json();
  return parseResponse<QuorumProgress>(json);
}
